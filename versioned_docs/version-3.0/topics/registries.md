...
A [Helm repository](./chart_repository) is a way to house and distribute packaged Helm charts. An OCI-based registry can contain zero or more Helm repositories and each of those repositories can contain zero or more packaged Helm charts.
...
The [`helm-sigstore`](https://github.com/sigstore/helm-sigstore) plugin allows using [Sigstore](https://sigstore.dev/) to sign Helm charts with the same tools used to sign container images.  This provides an alternative to the [GPG-based provenance](./provenance) supported by classic [chart repositories](./chart_repository).
...
If you have created a [provenance file](./provenance) (`.prov`), and it is present next to the chart `.tgz` file, it will
...
Users of the [helm-push plugin](https://github.com/chartmuseum/helm-push) (for uploading charts to [ChartMuseum](./chart_repository#chartmuseum-repository-server))
...
Migrating from classic [chart repositories](./chart_repository)
